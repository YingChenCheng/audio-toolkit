...
int main() {
    ...
    auto lpf_filter = std::make_shared<BiquadFilterNode>(BiquadFilterNode::Type::LowPass, 400.0f,
                                                         0.707f, 48000.0f);
    ...
    if (engine.start()) {
        std::cout << "Playing 880Hz Sine Wave with 400Hz Low-Pass Filter (muted high tone)..."
                  << std::endl;
